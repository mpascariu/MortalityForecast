...
#' \code{\link{getMortalityModels}} function.
#' @param ... Further argumets to be passed to other methods.
#' @inheritParams CoDa::predict.coda
#' @export
predict.MortalityModels <- function(object, h, ci = 95, 
  ...
  out <- list(call = match.call(), PLC = P1, codaLC = P2, LC = P3, 
              x = x, y = y)
  out <- structure(class = "predict.MortalityModels", out)
  return(out)
}
...
#' @inheritParams fitted.MortalityModels
#' @export
#' 
